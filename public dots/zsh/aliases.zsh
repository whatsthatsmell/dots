# Custom ZSH Aliases
alias h='hx'
alias -s {graphql,gql}=hx
alias -s txt=hx
alias -s vim=nvim
alias -s zsh=hx
alias -s zshrc=hx
alias -s {c,h}=hx
alias -s {js,json,hjson}=hx
alias -s {md,MD}=hx
alias -s {rs,toml,ron}=hx
alias -s {yml,yaml}=hx
alias -s lua=hx
...
# TODO: remove backwards compat aliases for exa/eza
# TODO: finish migration to eza
...
# Github repo search and go!
alias ghrs='gh s | xargs -n1 gh browse -R'
...
alias keys='bat ~/notes/key_binds_maps_mac.txt'
alias ldot='exa -ld .*'
...
alias nn="nvim -c ':exe \"normal iTdate\<Esc>o\"' -c ':lcd ~/notes'"
...
alias th='tmux new-window -c ~ -n 🏠Home'
