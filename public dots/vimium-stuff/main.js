...
chrome.runtime.onInstalled.addListener(function ({ reason }) {
  ...
  return chrome.tabs.query({ status: 'complete' }, function (tabs) {
    for (const tab of tabs) {
      for (const [func, files] of jobs) {
        for (const file of files) { func(tab.id, { file, allFrames: contentScripts.all_frames }, checkLastRuntimeError) }
      }
    }
  })
})
...
var forCountTabs = (count, currentTab, callback) => chrome.tabs.query({ currentWindow: true }, function (tabs) {
  ...
  const startTabIndex = Math.max(0, Math.min(activeTabIndex, tabs.length - count))
  for (const tab of tabs.slice(startTabIndex, startTabIndex + count)) { callback(tab) }
})
...
var HintCoordinator = {
  ...
  unregisterFrame (tabId, frameId) {
    if (!this.tabState[tabId]) { return }
    if ((this.tabState[tabId].ports != null ? this.tabState[tabId].ports[frameId] : undefined) != null) { delete this.tabState[tabId].ports[frameId] }
