  ...
  schedule(function()
    ...
    for win, winstate in pairs(windows) do
      ...
      elseif cfg.force and lastline and winstate.cursor[1] > lastline then
        if cfg.forcemark then
          vim.fn.setpos("'" .. cfg.forcemark, vim.fn.getcurpos())
        end
...
function M.handle_new()
  schedule(function()
    ...
    if
      not (
        npcall(api.nvim_win_get_var, 0, "previewwindow")
        or vim.tbl_contains(cfg.ignore.filetype, ft)
        or vim.tbl_contains(cfg.ignore.buftype, bt)
      )
    then
...
function M.setup(setup_cfg)
  ...
  cmd [[
	augroup Stabilize
		autocmd!
		autocmd WinNew * :lua require('joel.stabilize').handle_new()
		autocmd WinClosed * :lua require('joel.stabilize').handle_closed(tonumber(vim.fn.expand("<afile>")))
		autocmd CursorMoved,CursorMovedI * :lua require('joel.stabilize').save_window()
		autocmd User StabilizeRestore :lua require('joel.stabilize').restore_windows()
	augroup END
	]]
end
